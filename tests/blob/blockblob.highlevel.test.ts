...
describe("BlockBlobHighlevel", () => {
  ...
  it("download should success when internal stream unexpected ends at the stream end @loki @sql", async () => {
  ...
  // tslint:disable-next-line: max-line-length
  it("blockBlobClient.download should download full data successfully when internal stream unexpected ends @loki @sql", async () => {
    const uploadResponse = await blockBlobClient.uploadFile(tempFileSmall, {
      blockSize: 4 * 1024 * 1024,
      concurrency: 20
    });
  ...
  it("blockBlobClient.download should download partial data when internal stream unexpected ends @loki @sql", async () => {
    const uploadResponse = await blockBlobClient.uploadFile(tempFileSmall, {
      blockSize: 4 * 1024 * 1024,
      concurrency: 20
    });
  ...
  it("blockBlobClient.download should download data failed when exceeding max stream retry requests @loki @sql", async () => {
    const uploadResponse = await blockBlobClient.uploadFile(tempFileSmall, {
      blockSize: 4 * 1024 * 1024,
      concurrency: 20
    });
