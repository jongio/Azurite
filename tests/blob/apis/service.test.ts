...
describe("ServiceAPIs", () => {
  ...
  const baseClient = `http://${server.config.host}:${server.config.port}/devstoreaccount1`;
  ...
  it("List containers with default parameters @loki @sql", async () => {
    const result = (await serviceClient
      .listContainers()
      .byPage()
      .next()).value;
  ...
  it("List containers with all parameters configured @loki @sql", async () => {
    const containerNamePrefix = getUniqueName("container");
