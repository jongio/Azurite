...
describe("BlobAPIs", () => {
  ...
  it("get properties response should not set content-type @loki @sql", async () => {
    const blobClient404 = containerClient.getBlobClient("UN_EXIST_BLOB_");
    try {
      await blobClient404.getProperties();
    } catch (err) {
      assert.ok(!err.response.headers.get("content-type"));
    }

    try {
      await blobClient404.download(0, 0);
    } catch (err) {
  ...
  it("setTier set default to cool @loki @sql", async () => {
    ...
    assert.equal(
      true,
      listResult.segment.blobItems[0].properties.accessTierInferred
    );
  ...
  it("Copy blob should not override destination Lease status @loki", async () => {
    ...
    const sourceblobClient = containerClient.getBlockBlobClient(sourceBlob);
    const destblobClient = containerClient.getBlobLeaseClient();

    await sourceblobClient.upload("hello", 5);
    // await destblobClient.upload("hello", 5);

    const leaseResult = await destblobClient.acquireLease(-1);
    const leaseId = leaseResult.leaseId;
    assert.ok(leaseId);

    const getResult = await containerClient.getProperties();
    ...
    // await destblobClient.CopyFromURL(sourceblobClient.url, {
    //   conditions: { leaseAccessConditions: { leaseId } }
    // });
    ...
    await destblobClient.releaseLease();
  });
